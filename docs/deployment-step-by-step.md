...
After the ARM deployment finishes, you still grant Defender for Endpoint app roles if they were not handled by the permission script, authorize any non-MDE API connections that need sign-in, create the watchlist, upload Live Response library files, and run one test.
...
The current commercial templates do not use the WDATP connector for MDE operations, do not use `azuremonitorlogs` for queue polling, and do not use an Azure Blob connector for evidence upload. MDE and queue polling calls use raw HTTP with managed identity, and the endpoint uploads directly to the evidence container with the SAS URL generated by the broker.

For GCC High email notifications, the Office 365 connection should use `Office 365 Credentials (Office GCC High)`. The GCCH wrappers pass the `oauthGccHigh` Office 365 connector parameter set automatically.

If a non-MDE connection is not authorized, the Logic App can deploy but fail at runtime.
