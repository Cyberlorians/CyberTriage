...
The current commercial templates do not use the WDATP connector for MDE operations and do not use an Azure Blob connector for evidence upload. MDE calls use raw HTTP with managed identity, and the endpoint uploads directly to the evidence container with the SAS URL generated by the broker.
