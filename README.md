...
> the selected region does not have enough Azure Functions/App Service
> Consumption capacity for the SAS broker Function App. This is a subscription
> quota issue, not a template problem. The ARM template creates a Linux
> Function App on the `Y1` / `Dynamic` plan, and Azure may show the failing
> quota as `Dynamic VMs` even though the portal quota row is usually under App
> Service / Microsoft.Web.
>
> Fix it one of two ways:
>
> 1. Select a different region that already has App Service `Y1 VMs` quota.
> 2. Request a quota increase for App Service `Y1 VMs` in the target region. A
>    fresh deployment needs at least `1`; request more if the subscription will
>    host additional Function Apps.
>
> In the Azure portal, open **Subscriptions** > your subscription > **Usage +
> quotas**. Change the provider/filter from **Compute** to **App Service** or
> **Microsoft.Web**, select the deployment region, and search for `Y1` or `Y1
> VMs`. Request an increase for `Y1 VMs`. If the portal also shows `Total
> Regional VMs` under App Service / Microsoft.Web, request enough there too.
> After quota is approved, rerun **Review + create**.
