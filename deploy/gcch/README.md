# Azure Government / GCCH Deployment Guide

This guide walks through the CyberTriage Azure Government / GCCH deployment one step at a time.

Use this guide when deploying from the Azure Government portal:

```text
https://portal.azure.us
```

Do not use the commercial Azure portal for GCCH deployments.
...
```text
1. Azure Government subscription access.
...
For the validation tenant used while writing this guide, the Azure Government
account was:

```text
Tenant: ae1bccf2-c2af-49f4-882b-105dc11c5698
Subscription: 99c69aca-874f-48fe-ab6c-3e0f88f205f1
Sentinel workspace resource group: Sentinel
Sentinel workspace name: dibsecus
Sentinel workspace customer ID: 5714fa24-7a6b-4304-9d42-a0173a2eaede
...
Open this button in a browser where you are signed in to Azure Government:
...
If it opens `https://portal.azure.com`, stop and switch to the Azure Government
portal.
...
|---|---|---|
| Subscription | The subscription where you want the CyberTriage playbooks, Function App, and storage accounts deployed. | `Subscription 1` / `99c69aca-874f-48fe-ab6c-3e0f88f205f1` |
| Resource group | The resource group where CyberTriage resources will be created. Create a new one if this is a test. | `rg-cybertriage-gcch` |
| Region | Azure Government region for the CyberTriage resources. | `(US) USGov Virginia` |
| Location | Keep the generated value matching the region. | `usgovvirginia` |
...
| Example: `security-team@contoso.us` | The workflow skips the email action. |
...
For this validation tenant, the values are:

| Portal Field | Value For This Tenant |
|---|---|
| Sentinel Workspace Resource Group | `Sentinel` |
| Sentinel Workspace Name | `dibsecus` |
| Sentinel Workspace Customer Id | `5714fa24-7a6b-4304-9d42-a0173a2eaede` |
| Sentinel Workspace Subscription Id | `99c69aca-874f-48fe-ab6c-3e0f88f205f1` |
...
Most remaining fields should stay at their defaults for GCC High.
...
| Defender Api Base Uri | `https://api-gov.securitycenter.microsoft.us` |
| Defender Api Audience | `https://api-gov.securitycenter.microsoft.us` |
| Arm Base Uri | `https://management.usgovcloudapi.net` |
| Arm Audience | `https://management.usgovcloudapi.net/` |
...
For regular GCC, not GCC High, Microsoft documents the Defender API endpoint as:
...
If you are testing in the validation tenant used for this repo, the ARM form
should look like this. Replace only the globally unique names if Azure says a
name is already taken.

| Portal Field | Example Value |
|---|---|
| Subscription | `99c69aca-874f-48fe-ab6c-3e0f88f205f1` |
| Resource group | `rg-cybertriage-gcch-test` |
...
Do not copy `Sentinel`, `dibsecus`, or the workspace customer ID into a customer
tenant unless those are truly the customer's workspace values.
...
Run this script from the repo root in an Azure Government PowerShell session:
...
For this validation tenant, the Sentinel part would be:

```powershell
  -SentinelResourceGroup 'Sentinel' `
...
Open each Logic App in the Azure Government portal.
...
```text
https://<function-app-name>.azurewebsites.us/api/health
```
...
should include a `sasUrl` that starts like this in GCCH:

```text
https://<destination-storage-account>.blob.core.usgovcloudapi.net/cybertriage-results?
```

That proves the broker can create a user-delegation SAS in Azure Government.
...
If there are no onboarded active MDE devices in GCCH, stop after broker,
connection, and watchlist validation. Live Response cannot be tested without an
active device.
